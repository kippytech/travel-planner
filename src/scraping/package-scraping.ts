...
export const startPackageScraping = async (page: Page, pkg: PackageType) => {
  ...
  const packageDetails = await page.evaluate(() => {
    ...
    packageDetails.description = packageElement
      ...
      ?.innerHTML.replace(regex, "easy-book") as string;
    ...
    const descriptions: DetailedItenaryType[] = [];
    ...
    packageDetails.detailedItenary = descriptions;

    // For destination Iteniary
    const destinationItenary: { place: string; totalNights: number }[] = [];
    const destinationItenarySelector =
      packageElement?.querySelectorAll(".type-list li");

    destinationItenarySelector?.forEach((element) => {
      ...
      destinationItenary.push({ place: placeText!, totalNights });
    });

    packageDetails.destinationItenary = destinationItenary;
    ...
    const dataExtracted: PackageItenaryType[] = [];
    ...
    packageDetails.packageItenary = dataExtracted;
